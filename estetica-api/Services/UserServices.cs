...
using static dentist_panel_api.Utils.Constants;
...
namespace dentist_panel_api.Services
{
    public class UserServices : ControllerBase
    {
        ...
        private async Task<AuthResponse> BuildToken(ApplicationUser user)
        {
            ...
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(Environment.GetEnvironmentVariable("ESTETICA_JWT_KEY")));
            ...
            response.AuthState = mapper.Map<ApplicationUserDTO>(user);
